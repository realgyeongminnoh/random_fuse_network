class EdgeList:
    def __init__(self, length: int):
        ...
        self.idxMapFromEdgesToEdgesDivCap = []
        self.idxMapFromEdgesDivCapToEdges = []
        self.idxMapFromEdgesToEdgesDivCond = [] 
        self.idxMapFromEdgesDivCondToEdges = []
        self._GenerateEdges()
        self._GenerateEdgesDiv()
        self._GenerateEdgesDivCapCond()
        self._GenerateIdxMapForCap()
    ...
    def _GenerateIdxMapForCap(self):
        # edgesDivCap[idxMap[idxEdge]] -> edges[idxEdge]
        self.idxMapFromEdgesToEdgesDivCap = sorted(range(self.numEdge), key=lambda idxEdge: self.edgesDivCap[idxEdge][1])

        # edges[idxMap[idxEdgeCapDiv]] -> edgesDivCap[idxEdgesDivCap]
        self.idxMapFromEdgesDivCapToEdges = [None] * self.numEdge
        for idxEnum, idxInv in enumerate(self.idxMapFromEdgesToEdgesDivCap):
            self.idxMapFromEdgesDivCapToEdges[idxInv] = idxEnum


    def _GenerateIdxMapForCond(self):
        # edgesDivCond[idxMap[idxEdge]] -> edges[idxEdge]
        self.idxMapFromEdgesToEdgesDivCond = sorted(range(self.numEdge), key=lambda idxEdge: self.edgesDivCond[idxEdge][1])

        # edges[idxMap[idxEdgeCondDiv]] -> edgesDivCond[idxEdgesDivCond]
        self.idxMapFromEdgesDivCondToEdges = [None] * self.numEdge
        for idxEnum, idxInv, in enumerate(self.idxMapFromEdgesToEdgesDivCond):
            self.idxMapFromEdgesDivCondToEdges[idxInv] = idxEnum