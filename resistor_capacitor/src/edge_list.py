...
class EdgeList:
    ...
    def _GenerateIdxMapForCap(self):
        ...
        self.idxMapFromEdgesToEdgesDivCap = np.array(sorted(range(self.numEdge), key=lambda idxEdge: self.edgesDivCap[idxEdge][1]), dtype=np.int32)

        # edges[idxMap[idxEdgeCapDiv]] -> edgesDivCap[idxEdgesDivCap]
        self.idxMapFromEdgesDivCapToEdges = np.empty(self.numEdge, dtype=np.int32) 
