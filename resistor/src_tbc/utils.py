...
def get_configs_seeds() -> tuple[dict, dict]:
    ...
    path_data = Path("/disk/disk3/gyeongmin/project/random_fuse_network/resistor/data")
...
def check_volts_ext_seeds(dict_length_width_to_seeds) -> list[tuple[int, float, int]]:
    ...
    def check_one(tup):
        ...
        path_npy = path_data / str(length) / str(width) / str(seed) / "volts_ext.npy"
...
def check_connectivity_seeds(dict_length_width_to_seeds, dict_length_width_to_num_seed) -> list[tuple[int, float, int, int]]:
    """
    [all seeds in data] identify simulations with primal graph connected components greater than 2 
    """
    lengths_unique = np.unique(np.array(list(dict_length_width_to_num_seed))[:, 0]).astype(np.int32)
    path_data = Path(__file__).resolve().parents[1] / "data"
    record = []
    ...
    for length in lengths_unique:
        ...
        for width in [w for (l, w) in list(dict_length_width_to_num_seed) if l == length]:
            global path_length_width, width_global
            path_length_width = path_data / str(length) / str(width)
            width_global = width
